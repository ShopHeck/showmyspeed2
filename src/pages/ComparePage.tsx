  ...
  return (
    <div className="flex items-center gap-1.5">
      <div className="flex items-center gap-0.5">
        {[1, 2, 3, 4, 5].map(s => (
          <Star key={s} className={`w-3 h-3 ${s <= Math.round(rating) ? 'text-yellow-400 fill-yellow-400' : 'text-white/10'}`} />
        ))}
      </div>
...
function SortIcon({ col, active, dir }: { col: SortKey; active: SortKey; dir: SortDir }) {
  if (col !== active) return <ChevronsUpDown className="w-3.5 h-3.5 text-muted-foreground/30 ml-1 flex-shrink-0" />
  return dir === 'asc'
...
export function ComparePage() {
  ...
  return (
    <div className="min-h-screen pt-20 pb-32 px-4 sm:px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <motion.div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-8" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}>
          {summaryStats.map((stat, i) => (
            ...
            >
              ...
              <div className="text-xs text-muted-foreground/40 mt-0.5">{stat.sub}</div>
            </motion.div>
          ))}
        </motion.div>
        ...
        <motion.p className="text-xs text-muted-foreground/60 mb-5 flex items-center gap-1.5" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.18 }}>
          <BarChart3 className="w-3.5 h-3.5 flex-shrink-0" />
          Select up to 4 providers to compare side by side
        </motion.p>

        {/* Desktop table */}
        <motion.div className="hidden lg:block glass-card overflow-hidden mb-6" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                    ...
                    return (
                      <>
                        <motion.tr
                          ...
                          className={`border-b border-white/[0.04] cursor-pointer transition-colors hover:bg-white/[0.03] ${isp.highlight ? 'bg-primary/[0.03]' : ''} ${isExpanded ? 'bg-white/[0.04]' : ''}`}
                        >
                          <td className="px-3 py-4" onClick={e => e.stopPropagation()}>
                            <button onClick={() => !isDisabled && toggleCompare(isp.name)} disabled={isDisabled} className={`w-5 h-5 rounded flex items-center justify-center border transition-all flex-shrink-0 ${isChecked ? 'bg-primary border-primary text-primary-foreground' : isDisabled ? 'border-white/[0.06] text-white/10 cursor-not-allowed opacity-40' : 'border-white/[0.12] text-muted-foreground hover:border-primary/50 hover:text-primary'}`}>
                          ...
                          <td className="px-5 py-4">
                            <div>
                              <div className="font-semibold text-foreground flex items-center gap-2">
                                {isp.name}
                                {isp.highlight && <span className="text-xs px-1.5 py-0.5 rounded bg-primary/15 text-primary font-medium border border-primary/20">★</span>}
                              </div>
                          ...
                          <td className="px-5 py-4 text-right">
                            <span className="text-base font-display font-black gradient-text">
                        ...
                        <AnimatePresence>
                          {isExpanded && (
                            <motion.tr key={`${isp.name}-detail`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                              <td colSpan={11} className="px-5 pb-5 pt-1 bg-white/[0.02] border-b border-white/[0.04]">
                                <div className="grid grid-cols-3 gap-6">
                                  ...
                                  <div>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                      {isp.bestFor.map(tag => (
                                        <span key={tag} className="inline-flex items-center gap-1 text-xs px-2 py-1 rounded-lg border border-white/[0.08] bg-white/[0.03] text-muted-foreground">
          ...
          <div className="px-5 py-3 border-t border-white/[0.04] bg-white/[0.01] text-xs text-muted-foreground">
          ...
          {sorted.map((isp, i) => {
            ...
            return (
              <motion.div key={isp.name} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                className={`glass-card overflow-hidden ${isp.highlight ? 'border-primary/30' : ''}`}
              >
                <div className="p-4">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="text-right">
                      <div className="text-lg font-display font-black">${isp.price}<span className="text-xs font-normal text-muted-foreground">/mo</span></div>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => isAuthenticated ? saveProvider.mutate(isp) : toast.error('Sign in to save')} className={`p-2 rounded-lg border border-white/[0.08] ${isSaved ? 'text-primary' : 'text-muted-foreground'}`}>
                      {isSaved ? <BookmarkCheck className="w-4 h-4" /> : <Bookmark className="w-4 h-4" />}
                    </button>
                  </div>
                </div>
      ...
      {selectedForCompare.length >= 1 && (
        <div className="fixed bottom-0 inset-x-0 z-50 border-t border-white/[0.08] bg-card/95 backdrop-blur-xl p-3 sm:p-4">
          <div className="max-w-7xl mx-auto flex items-center gap-3 flex-wrap">
            <div className="flex items-center gap-2 flex-1 flex-wrap">
              {selectedForCompare.map(name => (
                <div key={name} className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-primary/10 border border-primary/20 text-primary text-xs font-medium">
                  {name}
            ...
            <button onClick={() => setSelectedForCompare([])} className="btn-ghost text-xs px-3 py-1.5">Clear</button>
            <button
